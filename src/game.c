...
void drawHealthBar(SDL_Renderer* renderer, int x, int y, int width, int height, float percent) {
...
void run_game() {
    ...
    // Initialisation des options de combat
    int battle_selected_option = 0;

    // Variables pour stocker le Pokémon sauvage sélectionné pour le combat
    ...
    while(!quit) {
        while(SDL_PollEvent(&e) != 0) {
            if(e.type == SDL_QUIT) {
                ...
                if(game_state == STATE_HOME) {
                    ...
                    if (rand() % 100 == 0) {
                        // Sélectionner un Pokémon sauvage aléatoire
                        int random_index = rand() % pokemon_count;
                        wild_pokemon = pokemons[random_index];  // Sélectionner le Pokémon
                        wild_pokemon.current_pv = wild_pokemon.total_pv;  // Initialiser ses PV actuels

                        // Charger la texture du sprite du Pokémon sauvage
                        ...
                        game_state = STATE_BATTLE;  // Changer d'état vers le combat
                    }
                }
                else if(game_state == STATE_BATTLE) {
                    if(e.key.keysym.sym == SDLK_DOWN) {
                        battle_selected_option = (battle_selected_option + 1) % 4;  // Il y a 4 options, donc on boucle
                    }
                    else if(e.key.keysym.sym == SDLK_UP) {
                        battle_selected_option = (battle_selected_option - 1 + 4) % 4;  // On remonte dans la liste
                    }
                    else if(e.key.keysym.sym == SDLK_RETURN) {
                        switch (battle_selected_option) {
                            case 0:  // Logique pour Attaquer
                                ...
                                break;
                            case 1:  // Logique pour Sac
                                break;
                            case 2:  // Logique pour Changer
                                break;
                            case 3:  // Logique pour Fuir
                                Mix_FreeMusic(battle_screen_background_music);
                                game_screen_background_music = Mix_LoadMUS("../assets/sound/music/game_screen_music.mp3");
                                Mix_PlayMusic(game_screen_background_music, -1);
                                game_state = STATE_GAME;  // Retourner au jeu
                                break;
                        }
                    }
                }
        ...
        if(game_state == STATE_HOME) {
            ...
            // Afficher le sprite du Pokémon du joueur (en bas à gauche)
            ...
            // Afficher le sprite du Pokémon sauvage (en haut à droite)
            ...
            drawHealthBar(renderer, 50, 370, 200, 20, (float)player.squad[0].current_pv / player.squad[0].total_pv);
            render_text(renderer, font, player.squad[0].nom, BLACK_COLOR, 50, 560);

            // Afficher la barre de vie du Pokémon sauvage
            drawHealthBar(renderer, 550, 20, 200, 20, (float)wild_pokemon.current_pv / wild_pokemon.total_pv);
            render_text(renderer, font, wild_pokemon.nom, BLACK_COLOR, 550, 210);

            // Afficher les options de combat (Attaquer, Sac, Changer, Fuir)
        ...
        SDL_Delay(16);  // Contrôle de la boucle de jeu (~60 FPS)
    }
